...
var AuditActivity = [
        ...
        { PaymentStatusName: "APPROVAL" },
          { PaymentStatusName: "ACTIVATION" },
          { PaymentStatusName: "DISCONNECTION" },
          { PaymentStatusName: "RECONNECTION" },
          { PaymentStatusName: "NEW CUSTOMER" },
          { PaymentStatusName: "IREPORT" }
                    ];
var MAPStatus = [
        { StatusName: "APPROVED" }, 
...
var MAP_Vendor = [{ Name: "ALL", Value: "ALL" },
...
var ApplicationStatus =
     [
     { PaymentStatusName: "HASARREARS" },
     { PaymentStatusName: "GOBRC" },
     { PaymentStatusName: "INSTALLMENTAL" },
     { PaymentStatusName: "PAYARREARS" },
     { PaymentStatusName: "ARREARSPERCENT" },
     { PaymentStatusName: "PROCEEDTOMAPPAY" },
     { PaymentStatusName: "MAPAPPLIED" },
     { PaymentStatusName: "APPROVED FOR PAYMENT" },
     { PaymentStatusName: "PAID FOR METER" },
     { PaymentStatusName: "MAPPAID" },
     { PaymentStatusName: "APPROVED FOR INSTALLATION" },
     { PaymentStatusName: "NEW" },
     { PaymentStatusName: "GOBRC" },
     { PaymentStatusName: "METER INSTALLED" },
     { PaymentStatusName: "VERIFY" }
     ];
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (data.CustomerDetails == null) {
                    ...
                    console.log("ddddddddddddddddddd "+self.CustomerDetails().BRCApprovalCS());
...
function ApprovePaymentViewModel(data) {
    ...
    self.ApplicationStatusList = ko.observableArray(ApplicationStatus);
    ...
    self.PaymentStatusList = ko.observableArray();
    ...
    self.ZonalApprovalStatus = ko.observable();
    ...
    self.GetContractorByVendor = function (data) {
        ...
        //  console.log("Hallelujsh Hosanna "+ self.SelectedVendor().Name);
    ...
    self.ViewInstalledMetersSignage = function (d) {

      //  console.log("Freedom " + self.SelectedStatus().PaymentStatusName);

        showloader(); 
        var Vendor = self.SelectedPaymentMode().Name; 
        $.ajax({
            type: "POST",
            url: '/MAP/ViewInstalledMetersSignage',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify({
                'Vendor': Vendor,
                'FromDate': self.FromDate(),
                'ToDate': self.ToDate(),
                'status': self.SelectedStatus().PaymentStatusName
            }),
            success: function (result) {
                var data = JSON.parse(result);
                hideloader();
    ...
    self.ReprintMAPForm = function (event) {

        var ReportViewer = "http://map.nepamsonline.com/ReportPage.aspx?";
        var TicketId = event.TransactionID();

        console.log("Ticketr  " + TicketId);

        var url = ReportViewer + "TicketId=" + TicketId + "&Type=OFFICEREPRINT";
        window.open(url, '_blank');
    ...
    self.ViewCustomersForInstallation = function (d) {
        ...
        var IBC = self.SelectedIBC().IBCName();
        var BSC = self.SelectedBSC().BSCName();
        var PaymentStatus = self.SelectedPaymentStatus().StatusName;

        var table = $('#customer_payment_info').DataTable();
        table.destroy();
        $.ajax({
            type: "POST",
            url: '/MAP/ViewCustomersForInstallation',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify({
                'IBC': IBC,
 ...
 PaginateTable('customer_payment_info3');
            },
            error: function (err) {
                dd("Fail Details Request " + err.responseText)
                swal("Error", "This Request Could not be Completed, if it Continues Please Contact IT", "error");
  ...
  self.fetchCustomerPaymentInfo2 = function () {
        ...
        $.ajax({
            type: "POST",
            url: '/MAP/FetchCustomerDataForApproval2',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            data: JSON.stringify({
                in_data: JSON.stringify(data),
